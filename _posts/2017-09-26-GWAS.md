...
The primary goal of this project is to identify which regions of the genome harbor SNPs that affect the phenotype, considering the concepts discussed in class.  The data set is the same as that we have used during the class. In the package, the environments were labeled as ``1``,``2``,``4`` and ``5``. You can check it using the ``head (wheat.Y) `` command. During the lesson, we used the environment ``5`` (last in the phenotype matrix) for the association analysis. **Each student has been assigned to an environment (1,2 or 4) that should be analyzed for his/her project** . In this [link](https://github.com/hos6236/hos6236.github.io/blob/master/classes/gwas_8.csv), you can see the trait assigned to you. If you plan to use the codes shown in the tutorial, be sure to change the argument ``Y = wheat.Y [, 4]``. This command defines the environment considered for association analysis.
